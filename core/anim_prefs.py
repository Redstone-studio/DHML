...
PRESETS = {
    ...
    "slide_up": AnimationPreset(
        key="slide_up",
        name=tr("下方滑入"),
        description=tr("从下面升上来，收尾绵长（easeOutQuint）"),
    ...
    "slide_left_bounce": AnimationPreset(
        ...
        name=tr("左侧滑入（弹簧）"),
        description=tr("从左边滑进来，用弹簧的方式过冲一下再自己收住（连贯，不是两段拼的）"),
        direction=DIR_HORIZONTAL,
        duration=760,
        offset=120,
        interval=115,
    ...
    "pcl_like": AnimationPreset(
        key="pcl_like",
        name=tr("仿 PCL2（克制）"),
        description=tr("位移很短、不回弹、收尾绵长 —— 最接近原生控件的那种顺滑"),
        direction=DIR_VERTICAL,
        duration=340,
        offset=28,
        interval=55,
    ...
    "slide_up_elastic": AnimationPreset(
        ...
        name=tr("下方滑入（弹性）"),
        description=tr("从下面升上来，像弹簧一样来回弹几下才停（参数化 elastic）"),
        direction=DIR_VERTICAL,
        duration=900,
        offset=110,
        interval=95,
    ...
    "slide_left_back": AnimationPreset(
        key="slide_left_back",
        name=tr("左侧滑入（过冲 18%）"),
        description=tr("滑过头 18% 再退回来。过冲量可调，不是 Qt 那个写死的 10%"),
        direction=DIR_HORIZONTAL,
        duration=520,
        offset=120,
        interval=85,
    ...
    "pcl_recipe": AnimationPreset(
        key="pcl_recipe",
        name=tr("PCL 原味（拆两条 + 淡入）"),
        description=tr("照 PCL 的真实配方：位移拆成快、慢两条并行曲线，同时淡入。"
                       "最自然的一档，建议先试它"),
        direction=DIR_VERTICAL,
        duration=350,
        offset=16,
...
def direction_text(direction: str) -> str:
    return tr("水平（左右）") if direction == DIR_HORIZONTAL else tr("垂直（上下）")
...
SPEEDS = {
    ...
    "normal": (1.0, tr("标准（1×）")),
    "slow": (1.5, tr("慢（1.5×）")),
    "very_slow": (2.2, tr("很慢（2.2×）")),
    "extreme": (3.2, tr("夸张（3.2×）")),
}
