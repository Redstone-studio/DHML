...
## 偏移是怎么挪出来的
    ...
    外层容器（父布局摆它的矩形，这个不动）
      └── 直接被 move() 挪走的卡片   ← 这一层没有布局，孩子归它自己管

这一层**不建布局**，孩子的偏移由 `move()` 直接写。为什么不靠布局内边距：
外层是普通的 QWidget（不进父布局的尺寸计算），内层自己 move 孩子，
两件事互不干扰 —— 父布局每轮 setGeometry() 只会摆**这一层**，
摆不到里面那个已经被挪走的孩子身上。

⚠️ 早期的写法是"给这一层的布局设负内边距"把卡片推出去。那招在 Qt 6.11 上
**已经失效**（`setContentsMargins()` 收到负数会退回样式默认值），详见
`SlideInRow` 的类注释。

溢出到容器外面的部分由祖先滚动区的 viewport 裁掉，视觉上就是从那一侧滑进来。
...
from PyQt6.QtCore import (
    ...
    QParallelAnimationGroup, QSize, QVariantAnimation, pyqtProperty, pyqtSignal,
)
...
class SlideInRow(QWidget):
    ...
    ## ⚠️ 和实验项目**不一样**的一处（Qt 6.11 实测推翻了原来那招）

    实验项目是"给这一层的布局设**负内边距**"把卡片推出去的。那个办法
    在 Qt 6.11 上**完全失效**：`QLayout.setContentsMargins()` 收到负数会
    退回样式默认值（实测设 -120，读回来是 11），卡片一步都不会动 ——
    也就是说那套滑入在新 Qt 上其实一直没生效，只是没人盯着逐帧看。

    现在改成：**这一层没有布局**，直接 `move()` 它包着的那个控件
    （外层由父布局摆位置、内层自己管孩子的偏移，两边互不干扰）。
    溢出的部分照旧由祖先滚动区的 viewport 裁掉，观感就是"从那一侧滑进来"。
    """
    ...
    def __init__(self, child: QWidget, duration: int = DEFAULT_DURATION,
        ...
        if animate is None:
            # 动效总开关（"关掉动效省性能"）：关了就一行动画都不播
            animate = appearance.get_anim_enabled()
        self._animate = bool(animate)
        if fade is None:
            # get_anim_fade() 在总开关关掉时**永远返回 False**，
            # 所以"关了动效还挂着离屏合成"这种情况不会发生
        ...
        # ⚠️ **不建布局**：这一层的"挪动"是直接 move 孩子做的，
        # 有布局的话布局每一轮都会把孩子摆回原位、把动画抹掉（见类注释）
        ...
        # 挂着不用纯属白白拖慢滚动。
        # ⚠️ 关掉动效时**根本不要建它**：这个 effect 是这条链路上最贵的一环
        # （每张卡片一次离屏合成），"关动效"要是还挂着它就没意义了。
        self._fade = None
        self._fade_anim = None
        if fade and self._animate:
    ...
    # ---------- 尺寸：这一层没有布局，得自己把孩子的尺寸接上去 ----------

    def _child_alive(self) -> bool:
        """包着的孩子还在吗

        ⚠️ 页面清列表时是"先 deleteLater() 删卡片、包装层晚一步才被删"，
        这中间布局还会给包装层派 resizeEvent —— 直接去摸 `self._child`
        会 `RuntimeError: wrapped C/C++ object ... has been deleted`。
        这种时候什么都不做就行（这一层自己也在被删的路上）。
        """
        try:
            self._child.isVisible()
        except RuntimeError:
            return False
        return True

    def _clamp_height(self, height: int) -> int:
        """把孩子想要的高度夹进它自己的 min/max 之间"""
        return max(self._child.minimumHeight(),
                   min(int(height), self._child.maximumHeight()))

    def _child_height(self) -> int:
        """孩子**实际**能占的高度

        ⚠️ 不能直接拿 `sizeHint()` 当这一层的高度：卡片常常自己
        `setFixedHeight()` 过（`_ModCard` 是 96），而它的 `sizeHint()` 会跟着
        里面描述文字的长短在 94~122 之间飘。布局按 sizeHint 给高度、卡片却
        咬死在 96，于是：

            sizeHint 更大（122）→ 每行多出一截空白，看着就是"行距莫名其妙变大"
            sizeHint 更小（94） → 卡片底部被这一层的边界切掉

        用户 2026-09 就是照着截图说"某些地方间距出问题了"。夹一层 min/max 之后
        跟卡片真正占的高度一致。
        """
        return self._clamp_height(self._child.sizeHint().height())

    def sizeHint(self):
        if not self._child_alive():
            return super().sizeHint()
        hint = self._child.sizeHint()
        return QSize(hint.width(), self._child_height())
    ...
    def minimumSizeHint(self):
        ...
        return QSize(hint.width(),
                     self._clamp_height(hint.height()))

    def resizeEvent(self, event):
        # 孩子宽度跟着这一层走、高度用它**自己定死的那个**（不是 sizeHint）
        if self._child_alive():
            height = self._child_height()
            if height > 0:
                self._child.resize(self.width(), height)
    ...
    def child(self) -> QWidget:
        """被包着的那个控件（外面这层只管"它在屏幕上的位置"）

        排查/测试要拿真身时用它 —— 直接扫布局拿到的是这一层包装，
        `isinstance(w, 某个卡片类)` 会一个都匹配不上（踩过）。
        """
        return self._child
    ...
    def reveal(self, on_done=None):
        ...
        关掉动效（总开关）时直接**摆到位**：不建动画、不起定时器、不挂离屏合成
        —— 这条路上一点开销都不留（见 `get_anim_enabled`）。
        """
        self.anim.stop()
    ...
    def stop(self):
        ...
        self._slow_offset = 0.0
        self._apply(0)
        # 总偏移也要归零：光 `_apply(0)` 只挪了孩子，`get_slide_offset()`
        # 还会读回起手那 120px，谁按它判断"这一行就位了没"就会判错
        self._slide_offset = 0.0
        if self._fade is not None:
            self.setGraphicsEffect(None)
            self._fade = None
...
class StaggerReveal:
    ...
    def __init__(self, parent: QWidget, interval: int = DEFAULT_INTERVAL,
                 ...
                 bounce_ratio: float = DEFAULT_BOUNCE_RATIO,
                 ease: str = DEFAULT_EASE, animate: bool = None):
        self._parent = parent
        if animate is None:
            from core import appearance
            animate = appearance.get_anim_enabled()
        # 关掉动效时**一次全放出来**（见 start()），所以"错峰间隔"这种东西
        # 这时候没有意义 —— 保留参数只是为了签名一致
        self._animate = bool(animate)
    ...
    def start(self):
        if not self._pending:
            return
        if not self._animate:
            # 关掉动效：**一次全放出来**。不能走定时器"一张一张点名" ——
            # 那样看着就是卡片每隔 X 毫秒蹦出来一个，比有动画还难受
            # （而且用户要的正是"别动"）。
            self._spawned.extend(self._pending)
            self._pending = []
