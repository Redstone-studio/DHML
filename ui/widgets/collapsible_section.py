...
def _anim_enabled() -> bool:
    """动效总开关（"关闭动效省性能"，见 core/appearance.py）

    读设置出问题就当开着 —— 展开动画这种东西，读不到设置不该让控件打不开。
    """
    try:
...
class CollapsibleSection(QWidget):
    ...
    def __init__(self, title: str, content: QWidget, parent=None,
        ...
        # ⚠️ 颜色**不能**在这里写死（原来是 #232428/#e9e9ec 那套深色）：
        # 控件自己的样式表优先级高于 app.qss 里的主题规则，从外面盖不掉 ——
        # 浅色主题下这一条会变成一根深色横条。样式在 60-dialogs.qss 的
        # `#SectionToggle` 里，颜色全是 @变量@。
    ...
    def set_expanded(self, expanded: bool, animate: bool = True):
        ...
        ⚠️ 动效总开关关掉时这里一律走"不放动画"那条路：用户要的是**别动**，
        留一个高度动画在这儿就成了"关了还剩一点"。
        """
        if animate and not _anim_enabled():
            animate = False
