...
class BackgroundCanvas(QWidget):
    ...
    def paintEvent(self, event):
        ...
        image = self._pixmap
        if image is None:
            # 没图：纯色。没设过就画**当前主题的页面色** —— 不能写死深灰，
            # 不然浅色主题下会画成一大块深色（实验项目那边写死的是 #1b1c1f）
            painter.fillRect(rect, QColor(self._settings.get("color")
                                          or self._theme_page_color()))
        else:
    ...
    @staticmethod
    def _theme_page_color() -> str:
        """当前主题的页面底色（QSS 里 `@bg_page@` 那份，单一来源）"""
        try:
            return (theme_state.palette().get("bg_page")
                    or appearance.FALLBACK_COLOR)
        except Exception:                                   # noqa: BLE001
            return appearance.FALLBACK_COLOR
