...
class ModsPage(TranslatableWidget):
    ...
    def _make_results_page(self) -> QWidget:
        ...
        # ---- 类型标签栏（模组 / 整合包 / 数据包 / 资源包 / 光影包）----
        # 照实验项目那套"类型标签页"：点一下就**直接重搜**（不用再点搜索按钮），
        # 标题和搜索按钮的文字也跟着变。
        # ⚠️ 用 QPushButton + `:checked`：QToolButton 的文字强制居中，
        # 而标签栏要左边第一个开始排。
        tabs = QHBoxLayout()
        tabs.setSpacing(6)
        self._type_buttons = {}
        for value, text in MOD_TYPES:
            btn = QPushButton()
            btn.setObjectName("ModTypeTab")
            btn.setCheckable(True)
            btn.setCursor(Qt.CursorShape.PointingHandCursor)
            btn.clicked.connect(
                lambda _checked, v=value: self.set_project_type(v))
            tabs.addWidget(btn)
